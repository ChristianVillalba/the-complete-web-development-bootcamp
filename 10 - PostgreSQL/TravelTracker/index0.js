...
const db = new pg.Client({
  ...
  host: "localhost",
  database: "world",
  password: "1234567", // remember to change your password
  port: 5432,
});
...
app.post("/add", async (req, res) => {  // new route > index.ejs > form - /add
  ...
  // We use the above input to make a query to our DB. and store it inside result
  const result = await db.query(    
    "SELECT country_code FROM countries WHERE country_name = $1",
    [input]   // changes the placeholder $1 for the user's input
  );
  // result.rows contains what are we looking for
  // if the result.rows is actually empty, it means we got a no match from this query.
